...
import { Sparkles, TrendingDown, Info, Calendar, ArrowRight } from 'lucide-react';
...
export const DebtProjection = memo(function DebtProjection({
  ...
  const activeStrategy = strategies.find(
    (strategy) => strategy.type === selectedStrategy
  ) || strategies[0];
  ...
  const chartData = activeStrategy?.projectionTimeline?.map((point) => ({
    date: point.date,
    ...
    return (
      <div className="p-6 rounded-xl bg-gradient-to-br from-gray-900/80 to-gray-900/40 border border-white/10 backdrop-blur-sm">
        <div className="flex justify-between items-center mb-5">
          <h2 className="text-xl font-semibold text-white">Debt Payoff Projection</h2>
        </div>
        <div className="bg-black/20 rounded-xl p-5 border border-white/5">
          <p className="text-white/70 text-center py-8">No debt payoff strategies available.</p>
        </div>
      </div>
    );
  }
      ...
      <div className="bg-black/20 rounded-xl p-5 border border-white/5">
        ...
        <div className="flex gap-2 mb-5 overflow-x-auto pb-1 custom-scrollbar">
          {strategies.map((strategy) => (
            ...
            >
              {strategy.type === 'avalanche' && <TrendingDown className="w-4 h-4 mr-2" />}
              {strategy.type === 'snowball' && <Calendar className="w-4 h-4 mr-2" />}
              {strategy.type === 'ai_optimized' && <Sparkles className="w-4 h-4 mr-2" />}
              {strategy.name}
            </Button>
          ))}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 mb-5">
          <div key="debt-free-date" className="bg-black/30 rounded-lg p-4">
            ...
            <p className="text-lg font-semibold text-white">
              {activeStrategy.projectedPayoffDate ? new Date(activeStrategy.projectedPayoffDate).toLocaleDateString('en-US', {
                month: 'long',
                year: 'numeric',
              }) : 'N/A'}
            </p>
            <div className="flex items-center mt-1">
              <Badge className="bg-[#88B04B]/20 text-[#88B04B] text-xs font-normal">
                {activeStrategy.projectedPayoffDate ? Math.ceil(
                  (new Date(activeStrategy.projectedPayoffDate).getTime() - new Date().getTime()) /
                    (1000 * 60 * 60 * 24 * 30)
                ) : 0} months
              </Badge>
            </div>
          ...
          <div key="monthly-payment" className="bg-black/30 rounded-lg p-4">
            ...
            <p className="text-lg font-semibold text-white">${activeStrategy.monthlyPayment ? activeStrategy.monthlyPayment.toLocaleString() : '0'}</p>
            <div className="flex items-center mt-1">
              <Badge className="bg-white/10 text-white/70 text-xs font-normal">
                +${activeStrategy.extraPaymentAmount ? activeStrategy.extraPaymentAmount.toLocaleString() : '0'} extra
              </Badge>
            </div>
          </div>
          ...
          <div key="interest-saved" className="bg-black/30 rounded-lg p-4">
            ...
            <p className="text-lg font-semibold text-white">${activeStrategy.interestSaved ? activeStrategy.interestSaved.toLocaleString() : '0'}</p>
            <div className="flex items-center mt-1">
        ...
        <div className="h-64 mb-4">
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={chartData} margin={{ top: 10, right: 10, left: 0, bottom: 20 }}>
              ...
              />
              <Tooltip 
                formatter={(value) => [`$${Number(value) ? Number(value).toLocaleString() : '0'}`, 'Debt Balance']}
                labelFormatter={(label) => label ? new Date(label).toLocaleDateString('en-US', {
                  month: 'long',
                  year: 'numeric'
                }) : 'Unknown Date'}
                contentStyle={{ 
        ...
        <div className="p-4 bg-black/20 rounded-lg border border-white/10">
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-white/70 mb-2">{activeStrategy.recommendation || 'No recommendation available'}</p>
              <Button
                variant="outline"
                size="sm"
