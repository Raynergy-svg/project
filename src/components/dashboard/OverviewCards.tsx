...
const generateTrendData = (
  ...
  const data = [];
  let currentDebt = startingDebt || 0;
  const payment = monthlyPayment || 0;
  const change = Math.abs(monthlyChange || 0);
  ...
  for (let i = 0; i < 6; i++) {
    ...
    currentDebt = Math.max(0, currentDebt - (payment + change));
  }
...
export const OverviewCards = memo(function OverviewCards({
  ...
  useEffect(() => {
    if (typeof window !== "undefined") {
      setTrendData(
        generateTrendData(totalDebt, totalMonthlyPayment, monthlyChange)
      );
  ...
  const formattedMonthlyPayment =
    typeof totalMonthlyPayment === "number"
      ? totalMonthlyPayment.toLocaleString()
      : "0";
  const formattedMonthlyChange =
          ...
          <div className="flex items-center justify-between mb-3">
            ...
            {monthlyChange < 0 && (
              <Badge className="bg-[#88B04B]/20 text-[#88B04B]">
                ...
                Decreasing (${formattedMonthlyChange}/mo)
              </Badge>
            )}
          </div>
