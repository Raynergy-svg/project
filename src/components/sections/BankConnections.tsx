...
export function BankConnections() {
  ...
  // Make sure this component runs only on client side
  useEffect(() => {
    setIsMounted(true);
  }, []);
  ...
  const handleInitializeDebtsTable = async () => {
    ...
    try {
      ...
      setInitError(
        "We encountered an issue setting up your account. Please try again."
      );
    } finally {
  ...
  const handleOpenPlaid = useCallback(async () => {
    ...
    try {
      ...
      const token = await openBankConnection();
      setLinkToken(token);

      // In a real implementation, you would open Plaid Link
      // For now, we'll simulate a mock connection
      setTimeout(() => {
        // Simulate connecting accounts
        const mockBankData = {
          institutionId: "ins_" + Math.random().toString(36).substring(2, 10),
  ...
  }, [isPlaidReady, openBankConnection, connectAccount]);
      ...
      if (
        ...
        const accountsToConnect = metadata.accounts.map((account) => ({
          ...
          balance:
            ...
            Math.floor(Math.random() * 10000) / 100, // Use real balance if available
        ...
        Promise.all(accountsToConnect.map((acc) => connectAccount(acc)))
          .then(() => {
            ...
            alert(
              `Successfully connected ${accountsToConnect.length} accounts from ${metadata.institution.name}`
            );
          ...
          .catch((err) => {
            ...
            alert(
              "There was an issue connecting your accounts. Please try again."
            );
        });
      }
    ...
    setTimeout(() => {
      ...
      Promise.all(mockAccounts.map((acc) => connectAccount(acc)))
        .then(() => {
          // Show a success message
          console.log(
            `Successfully connected ${mockAccounts.length} accounts from ${bank.name}`
          );
  ...
  const handleConnectBankAccount = (bankId: string) => {
    ...
    if (ready && linkToken) {
      ...
      open();
        ...
        setPlaidError(
          "There was an issue opening the bank connection flow. Please try again."
        );
      }
    } else {
  ...
  // If not mounted or running on server, render a placeholder
  if (!isMounted) {
    return (
      <div className="rounded-xl bg-gradient-to-br from-gray-900/80 to-gray-900/40 border border-white/10 backdrop-blur-sm p-6">
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-2">
            <div className="p-2 rounded-lg bg-[#88B04B]/20 border border-[#88B04B]/30">
              <Building className="w-4 h-4 text-[#88B04B]" />
            </div>
    ...
    <div className="mx-auto max-w-3xl">
      <div className="mb-8 text-center">
        ...
        <p className="text-white/70 max-w-lg mx-auto">
          Securely connect your bank accounts to automatically import your
          financial data and get personalized insights
        </p>
      </div>
    ...
    <Card className="overflow-hidden bg-black border border-white/10 shadow-sm">
      <CardHeader className="pb-3 border-b border-white/10">
        ...
        <CardDescription className="text-sm text-gray-400">
              Connect your bank accounts to analyze your debts and get
      ...
      <CardContent className="p-0">
          ...
          <h3 className="text-lg font-medium text-white mb-2">
            ...
            Connect your bank accounts securely with Plaid to enable AI-powered
            debt analysis and get personalized financial insights.
            </p>
            <Button
              onClick={handleOpenPlaid}
              size="lg"
        ...
        <DialogContent className="bg-gray-900 border-white/10 text-white">
          ...
          <div className="mt-4 pt-4 border-t border-white/10">
            ...
            <button
              ...
              onClick={() =>
                alert(
                  "This would open a search dialog in a full implementation"
                )
              }
            >
